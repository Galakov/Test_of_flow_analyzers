...
class DataDebuggerDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.tabs.addTab(self.problems_tab, '⚠️ Проблемы')
    ...
    def refresh_analysis(self):
        ...
        if not hasattr(self, 'data_files') or not self.data_files:
            self.structure_text.setText("❌ Нет загруженных файлов для анализа")
            self.analysis_text.setText("❌ Нет данных для анализа")
            self.problems_text.setText("❌ Нет данных для анализа проблем")
            return
...
class AnalyzerComparisonApp(QMainWindow):
    ...
    def create_data_table_panel(self, parent_splitter):
        ...
        # Инициализация переменных для выделения
    ...
    def debug_data_conversion(self, df, file_type):
        ...
        if data_cols and len(data_cols) > 0:
            ...
            print(f"📊 Анализ колонки: '{test_col}'")
            ...
            if problems:
                print(f"⚠️ НАЙДЕНЫ ПРОБЛЕМЫ ({len(problems)} случаев):")
                for idx, orig, conv in problems[:5]:
            ...
            if new_zeros.sum() > 0:
                print("⚠️ ВНИМАНИЕ: Появились новые нули!")
        ...
        for i, (gas_type, df, time_col, data_cols) in enumerate(plot_configs):
            ...
            if parsed.isna().all():
                ...
                if len(time_data) > 0:
                    ...
                    # Проверка на наличие данных после 16.11.2025 23:50
                    check_date = pd.to_datetime('16.11.2025 23:50', dayfirst=True)
                    if max_date > check_date:
                        after_check = (time_data > check_date).sum()
                        print(f"  ✓ Записей после {check_date}: {after_check}")
                        if after_check == 0:
                            print(f"  ⚠ ВНИМАНИЕ: Нет записей после целевой даты, хотя max_date > check_date!")
                    else:
                        print(f"  ⚠ ВНИМАНИЕ: Максимальная дата ({max_date}) раньше проверочной даты ({check_date})")
            ...
            for j, col in enumerate(data_cols):
                try:
                    ...
                    if len(timestamps) != len(numeric_values):
                        print(f"⚠ Несоответствие длины: timestamps={len(timestamps)}, values={len(numeric_values)}")
                    ...
                    if len(valid_values) > 0:
                        ...
                        print(f"  ✓ График построен с {len(valid_values)} точками")
                    else:
                        print(f"  ⚠ Нет данных для построения графика")
